...
long long ComputeDensitiesMT(int tid,int EventSet, long long values[], long long *thread_start, int *thread_papi_skip_counter)//,long long timestamp, long long start_time)
{
  ...
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz){
	...
	// ------ PAPI ------------ 
	if (iz % 2 == 0 ){
	  //timestamp = papi_read(EventSet,values,tid,timestamp, start_time); 
	  mt_waca_papi_read(EventSet,values,tid,thread_start,thread_papi_skip_counter);
...
long long ComputeForcesMT(int tid,int EventSet, long long values[], long long *thread_start, int *thread_papi_skip_counter)//long long timestamp, long long start_time)
{
  int neighCells[3*3*3];
	
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz){
	  
	   //timestamp = papi_read(EventSet,values,tid,timestamp, start_time); //--------------------------------PAPI--------------------------------------------------------------
	   mt_waca_papi_read(EventSet,values,tid,thread_start,thread_papi_skip_counter);
...
#else
//void ProcessCollisionsMT(int tid)
long long ProcessCollisionsMT(int tid,int EventSet, long long values[], long long *thread_start, int *thread_papi_skip_counter)//,long long timestamp, long long start_time)
{
	for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
	{
	//--------------------------------PAPI--------------------------------------------------------------
...
/* 	// Elapsed timestamp
	long long mt_elapsed[8], mt_timestamp[8], prev_time[8]; //Hard coded 8 value has to be romved to get total number of threads
	static long long mt_microseconds[8];
	...
	prev_time[tid]  = mt_timestamp[tid];

	mt_elapsed[tid] = current.tv_sec*NANOS + current.tv_nsec - start_time;
	mt_timestamp[tid] = mt_elapsed[tid] / 1000 + (mt_elapsed[tid] % 1000 >= 500); // round up halves

	mt_microseconds[tid] = mt_timestamp[tid] - prev_time[tid]; */
	// mt_microseconds[tid] = ( mt_timestamp[tid] )- mt_microseconds[tid]; 
...
//-----------------------------------------MULTI - PAPI----------------------------------------------------------------------------------------------------
  ...
  pthread_barrier_wait(&barrier);
  //mt_timestamp[tid] = ComputeDensitiesMT(tid,EventSet,values,mt_timestamp[tid], start_time);		//---PAPI: Compute intensive function > 3B instructions retired
  ComputeDensitiesMT(tid,EventSet,values,&thread_start,&thread_papi_skip_counter);
 
  pthread_barrier_wait(&barrier);
  ComputeDensities2MT(tid);
  
  pthread_barrier_wait(&barrier);
  //mt_timestamp[tid] = ComputeForcesMT(tid,EventSet,values,mt_timestamp[tid], start_time);		//---PAPI: Compute intensive function > 5B instructions retired
  ComputeForcesMT(tid,EventSet,values,&thread_start,&thread_papi_skip_counter);
 
  pthread_barrier_wait(&barrier);
  //mt_timestamp[tid] = ProcessCollisionsMT(tid,EventSet,values,mt_timestamp[tid], start_time);
  ProcessCollisionsMT(tid,EventSet,values,&thread_start,&thread_papi_skip_counter);//
...
//mt_timestamp[tid] = papi_read(EventSet,values,tid,mt_timestamp[tid], start_time); //--------------------------------PAPI--------------------------------------------------------------
mt_waca_papi_read(EventSet,values,tid,&thread_start,&thread_papi_skip_counter);
