...
// CPU Power
char CPU_TEMP[FILE_SIZE];
int CPU_FILE, CPU_READ;
char CPU_POWER[FILE_SIZE];
...
// CV Power
char CV_TEMP[FILE_SIZE];
int CV_FILE, CV_READ;
char CV_POWER[FILE_SIZE];

// DDR Power
char DDR_TEMP[FILE_SIZE];
int DDR_FILE, DDR_READ;
char DDR_POWER[FILE_SIZE];

// TOTAL Power
char TOTAL_TEMP[FILE_SIZE];
int TOTAL_FILE, TOTAL_READ;
char TOTAL_POWER[FILE_SIZE];
...
char TEMP_CPU6_TEMP[FILE_SIZE];
int TEMP_CPU6_FILE, TEMP_CPU6_READ;

char TEMP_CPU7_TEMP[FILE_SIZE];
int TEMP_CPU7_FILE, TEMP_CPU7_READ;
...
void log_data(long long *values, int tid, long long microseconds, long long timestamp){
		...
		// Power monitor - 1
		// Channel-0 is the GPU power
		GPU_FILE = open("/sys/bus/i2c/drivers/ina3221x/1-0040/iio:device0/in_power0_input", O_RDONLY);
		GPU_READ = read(GPU_FILE, GPU_POWER, sizeof(GPU_POWER));
		GPU_POWER[strlen(GPU_POWER)-1] = '\0'; 
		close(GPU_FILE);

		// Channel-1 is the GPU power
		CPU_FILE = open("/sys/bus/i2c/drivers/ina3221x/1-0040/iio:device0/in_power1_input", O_RDONLY);
		CPU_READ = read(CPU_FILE, CPU_POWER, sizeof(CPU_POWER));
		CPU_POWER[strlen(CPU_POWER)-1] = '\0'; 
		close(CPU_FILE);

		// Channel-2 is the total SoC power
		SOC_FILE = open("/sys/bus/i2c/drivers/ina3221x/1-0040/iio:device0/in_power2_input", O_RDONLY);
		SOC_READ = read(SOC_FILE, SOC_POWER, sizeof(SOC_POWER));
		SOC_POWER[strlen(SOC_POWER)-1] = '\0'; 
		close(SOC_FILE);

		// Power monitor - 2
		// Channel-0 is the CV power
		CV_FILE = open("/sys/bus/i2c/drivers/ina3221x/1-0041/iio:device1/in_power0_input", O_RDONLY);
		CV_READ = read(CV_FILE, CV_POWER, sizeof(CV_POWER));
		CV_POWER[strlen(CV_POWER)-1] = '\0'; 
		close(CV_FILE);

		// Channel-1 is the DDR power
		DDR_FILE = open("/sys/bus/i2c/drivers/ina3221x/1-0041/iio:device1/in_power1_input", O_RDONLY);
		DDR_READ = read(DDR_FILE, DDR_POWER, sizeof(DDR_POWER));
		DDR_POWER[strlen(DDR_POWER)-1] = '\0'; 
		close(DDR_FILE);

		// Channel-2 is the total board power
		TOTAL_FILE = open("/sys/bus/i2c/drivers/ina3221x/1-0041/iio:device1/in_power2_input", O_RDONLY);
		TOTAL_READ = read(TOTAL_FILE, TOTAL_POWER, sizeof(TOTAL_POWER));
		TOTAL_POWER[strlen(TOTAL_POWER)-1] = '\0'; 
		close(TOTAL_FILE);
		...
		TEMP_CPU6_FILE = open("/sys/devices/virtual/thermal/thermal_zone6/temp", O_RDONLY);
		TEMP_CPU6_READ = read(TEMP_CPU6_FILE, TEMP_CPU6_TEMP, sizeof(TEMP_CPU6_TEMP));
		TEMP_CPU6_TEMP[strlen(TEMP_CPU6_TEMP)-1] = '\0'; 
		close(TEMP_CPU6_FILE);

		TEMP_CPU7_FILE = open("/sys/devices/virtual/thermal/thermal_zone7/temp", O_RDONLY);
		TEMP_CPU7_READ = read(TEMP_CPU7_FILE, TEMP_CPU7_TEMP, sizeof(TEMP_CPU7_TEMP));
		TEMP_CPU7_TEMP[strlen(TEMP_CPU7_TEMP)-1] = '\0'; 
		close(TEMP_CPU7_FILE);
		...
		CPU6_UTIL_FILE = open("/sys/devices/system/cpu/cpufreq/ondemand/cpu_util_6", O_RDONLY);
		...
		CPU7_UTIL_FILE = open("/sys/devices/system/cpu/cpufreq/ondemand/cpu_util_7", O_RDONLY);
		...
		fprintf(fp,"%s,%s,%s,%s,%s,%s,%lld,%lld,%lld,%lld,%lld,%lld,%lld,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%d,%lld",
			GPU_POWER, CPU_POWER, SOC_POWER, CV_POWER, DDR_POWER, TOTAL_POWER,
			microseconds,
			values[0],values[1],values[2], values[3], values[4], values[5],
			TEMP_CPU0_TEMP,TEMP_CPU1_TEMP,TEMP_CPU2_TEMP,TEMP_CPU3_TEMP,TEMP_CPU4_TEMP,TEMP_CPU5_TEMP,TEMP_CPU6_TEMP,TEMP_CPU7_TEMP,
			CPU0_FREQ,
			CPU0_UTIL_VALUE,CPU1_UTIL_VALUE,CPU2_UTIL_VALUE,CPU3_UTIL_VALUE,CPU4_UTIL_VALUE,CPU5_UTIL_VALUE,CPU6_UTIL_VALUE,CPU7_UTIL_VALUE,
			CPU0_OF_VALUE,CPU1_OF_VALUE,CPU2_OF_VALUE,CPU3_OF_VALUE,CPU4_OF_VALUE,CPU5_OF_VALUE,CPU6_OF_VALUE,CPU7_OF_VALUE,
			tid, timestamp);
